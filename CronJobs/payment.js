...
async function processPayments() {
  try {
    ...
    for (const plan of duePlans) {
      try {
        ...
        combinedLogger.error(
          `Error processing Plan ${plan.planId}: ${JSON.stringify(err, Object.getOwnPropertyNames(err))}`
        )
      }
    }
    ...
    combinedLogger.error(
      `Fatal error in processPayments: ${JSON.stringify(err, Object.getOwnPropertyNames(err))}`
    )
    process.exit(1)
  }
}
